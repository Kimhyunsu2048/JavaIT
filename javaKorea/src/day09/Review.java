...
//		실수형변수 = Double.parseDouble(sc.nextLine()); // 입력받은 문자열을 실수로 변환(nextDouble 대체용)		
		...
		// String <-> int : 개발된 자료형(클래스) , String만 예외임
		// toString, Integer.parseInt
		String str= "001230";
		num = Integer.parseInt(str); // 문자열을 정수로 변환
		str = Integer.toString(num); // 정수를 문자열로 변환
...
class ReviewTest extends TheOne implements TheTwo {
	...
	public void go() {
		...
		for(int i=0;i<5;i++) { 		//지역변수 i는 중괄호가 끝나면 없어짐
...
class TheOne {
	...
	public String getStr() {	//메서드의 자료형 : return 의 자료형
...
interface TheTwo{
	public void go();	// 인터페이스의 메서드는 상속받은 곳에서 {}를 구현해줘야함
}
